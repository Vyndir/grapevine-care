...
| Call-out: evaluate coverage | `get_coverage_candidates({})` | Resolves the single active uncovered shift and returns the same eight named checks for Maya, Jordan, Luis, and Elena; only Jordan is eligible | Guessing when multiple shifts need coverage, sensitive-trait ranking, opaque suitability score, or hiding exclusions | Server owns deterministic constraints |
