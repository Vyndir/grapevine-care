...
import type { CareState, Dose, PreparedAction, ResidentResponseCode, Scenario } from "./schemas";
...
function CaregiverView({ state, actions, busy, onMessage }: { state: CareState; actions: CareActions; busy: boolean; onMessage(message: string): void; }) {
  ...
  return <main className="caregiver-layout">
    <section className="caregiver-main"><div className="workspace-heading"><div><p className="eyebrow">Caregiver workspace · fictional resident</p><h1>{state.resident.severity === "routine" ? "Rose is on schedule" : state.resident.severity === "urgent" ? "A device safety hold needs review" : "Rose’s care routine needs attention"}</h1><p>Evidence is separated from interpretation. You choose whether a prepared action should proceed.</p></div><span className={`severity-badge ${state.resident.severity}`}><i /> {state.resident.severity}</span></div>
      <section className={`attention-panel ${state.resident.severity}`}><div className="attention-icon">{state.resident.severity === "routine" ? <CheckCircleIcon weight="fill" /> : <WarningCircleIcon weight="fill" />}</div><div><p className="eyebrow">Current signal</p><h2>{state.events[0]?.summary}</h2><p>{state.events[0]?.detail}</p><dl><div><dt>Observed</dt><dd>{timeLabel(state.events[0]?.occurred_at, state.resident.timezone)}</dd></div><div><dt>Source</dt><dd>{state.events[0]?.source}</dd></div><div><dt>Meaning</dt><dd>{state.resident.scenario === "missed_window" ? "Removal was not confirmed; ingestion and welfare remain unknown." : state.resident.scenario === "device_offline" ? "Telemetry is stale; the local controller remains authoritative." : "A deterministic device rule produced this state."}</dd></div></dl></div></section>
      ...
      <section className="evidence-panel"><div className="section-heading"><div><p className="eyebrow">Evidence chain of custody</p><h2>What the system actually knows</h2></div><span>Actor · class · trust boundary</span></div><ol>{state.events.slice(0, 7).map((event) => <li key={event.id}><span className={`event-dot ${event.severity}`} /><div><strong>{event.summary}</strong><p>{event.detail}</p><small>{timeLabel(event.observed_at, state.resident.timezone)} · {event.actor_type.replaceAll("_", " ")} · {event.evidence_type.replaceAll("_", " ")} · {event.source}</small></div></li>)}</ol></section>
    </section>
    <aside className="caregiver-rail"><section className="action-panel"><p className="eyebrow">Human-controlled next step</p><h2>{openAction ? "One check-in is ready to review" : canPrepare ? "Prepare a check-in" : "Waiting for Rose"}</h2><p>{openAction ? "Nothing has been sent. Open the review panel below to approve or dismiss it." : canPrepare ? "Choose a channel to stage from current evidence. A person must still review it." : "A bounded question is available on Rose's station. The agent cannot answer on her behalf or skip this evidence step."}</p><div className="action-buttons"><button type="button" disabled={busy || Boolean(openAction) || !canPrepare} onClick={() => void stage("call")}>Prepare call</button><button type="button" disabled={busy || Boolean(openAction) || !canPrepare} onClick={() => void stage("visit")}>Prepare visit</button></div></section><section className="schedule-panel"><p className="eyebrow">Today’s care plan</p><h2>Medication windows</h2><ul>{state.doses.map((dose) => <li key={dose.id}><span><strong>{dose.label}</strong><small>{dose.window_label}</small></span><b className={dose.status}>{doseStatusLabel(dose.status)}</b></li>)}</ul></section><CareCircle /></aside>
  </main>;
}
