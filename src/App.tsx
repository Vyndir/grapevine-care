...
export default function App() {
  ...
  return <div className="care-shell"><header className="care-header"><Brand /><nav className="workspace-nav" aria-label="Choose workspace"><button className={workspace === "caregiver" ? "active" : ""} type="button" onClick={() => setWorkspace("caregiver")}><UsersThreeIcon />Caregiver cockpit</button><button className={workspace === "resident" ? "active" : ""} type="button" onClick={() => setWorkspace("resident")}><HouseIcon />Rose’s view</button><button className={workspace === "system" ? "active" : ""} type="button" onClick={() => setWorkspace("system")}><GaugeIcon />Devices & MCP</button></nav><button className="quiet-button" type="button" onClick={() => setWorkspace(pending || pendingHandoff ? "caregiver" : "resident")}>{pending || pendingHandoff ? <BellIcon /> : <HouseIcon />}{pending || pendingHandoff ? "1 review waiting" : "View Rose’s station"}</button></header>
