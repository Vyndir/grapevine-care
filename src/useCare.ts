...
export function useCare() {
  ...
  const getState = useCallback(() => api<CareState>("/api/care/state?resident_id=rose-demo"), []);
  ...
  const setScenario = useCallback(async (scenario: Scenario) => {
    setBusy(true);
    try {
      const result = await api<{ state: CareState }>("/api/care/scenario", { method: "POST", body: JSON.stringify({ scenario }) });
  ...
  const confirmDose = useCallback(async (doseId: string) => {
    setBusy(true);
    try {
      const result = await api<{ state: CareState }>(`/api/care/doses/${encodeURIComponent(doseId)}/confirm`, { method: "POST", body: "{}" });
      return commit(result.state);
    } finally { setBusy(false); }
  }, [commit]);
  ...
  const prepareAction = useCallback(async (input: { resident_id: string; channel: PreparedAction["channel"]; reason: string; idempotency_key: string; }) => {
    const result = await api<{ action: PreparedAction; approval_required: true; external_side_effect: false; }>("/api/care/actions", { method: "POST", body: JSON.stringify(input) });
    await refresh();
    return result;
  }, [refresh]);
  ...
  const resolveAction = useCallback(async (actionId: string, resolution: "approved_in_demo" | "dismissed") => {
    setBusy(true);
    try {
      const result = await api<{ state: CareState }>(`/api/care/actions/${encodeURIComponent(actionId)}/resolve`, { method: "POST", body: JSON.stringify({ resolution }) });
      return commit(result.state);
    } finally { setBusy(false); }
  }, [commit]);
