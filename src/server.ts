...
type DbTeamInquiry = { inquiry_id: string; resident_id: "rose-demo" | "walter-demo" | "evelyn-demo"; caregiver_id: string; inquiry_type: "visit_verification" | "shift_readiness" | "visit_outcome" | "handoff_approval" | "handoff_receipt"; prompt: string; status: "awaiting_coordinator_approval" | "response_received" | "resolved" | "dismissed"; response_code: "arrived_verification_failed" | "delayed" | "cannot_attend" | "no_response" | "ready_for_visit" | "visit_record_submitted" | "handoff_approved" | "handoff_received" | null; response_detail: string | null; idempotency_key: string; created_at: string; responded_at: string | null; resolved_at: string | null; };
...
function buildCareTeamDay(run: DbRun, shifts: DbShift[], packets: DbOrientationPacket[], inquiryRows: DbTeamInquiry[]): CareTeamDay | undefined {
  ...
  const jordanReadiness = inquiries.find((inquiry) => inquiry.resident_id === "rose-demo" && inquiry.inquiry_type === "shift_readiness");
  const jordanVisitOutcome = inquiries.find((inquiry) => inquiry.resident_id === "rose-demo" && inquiry.inquiry_type === "visit_outcome");
  const jordanHandoffApproval = inquiries.find((inquiry) => inquiry.resident_id === "rose-demo" && inquiry.inquiry_type === "handoff_approval");
  const luisHandoffReceipt = inquiries.find((inquiry) => inquiry.resident_id === "rose-demo" && inquiry.inquiry_type === "handoff_receipt");
  ...
  if (step >= 3) {
    ...
    attention.push({
      ...
      resident_id: "rose-demo",
      resident_name: "Rose",
      state: stage === "start" ? (visitStarted ? "resolved" : jordanReadiness ? "waiting_on_human" : "attention_now") : stage === "complete" ? (visitCompleted ? "resolved" : jordanVisitOutcome ? "waiting_on_human" : "attention_now") : (handoffAcknowledged ? "resolved" : roseShift?.handoff_status === "available_to_next_caregiver" ? "waiting_on_human" : "attention_now"),
      attention_reason: stage === "start" ? (visitStarted ? "Jordan’s readiness was verified and the visit began" : jordanReadiness?.status === "response_received" ? "Jordan replied; verify her readiness before the visit begins" : jordanReadiness ? "Jordan’s readiness check-in is awaiting coordinator action" : "Jordan is assigned; confirm her brief and visit readiness") : stage === "complete" ? (visitCompleted ? "Jordan’s submitted visit record was verified" : jordanVisitOutcome?.status === "response_received" ? "Jordan submitted the visit record; verify the bounded evidence" : jordanVisitOutcome ? "Jordan’s visit-update request is awaiting coordinator action" : "Request Jordan’s visit outcome before preparing the handoff") : (handoffAcknowledged ? "Luis’s handoff receipt was verified" : roseShift?.handoff_status === "available_to_next_caregiver" ? (luisHandoffReceipt?.status === "response_received" ? "Luis confirmed receipt; verify his response" : luisHandoffReceipt ? "Luis’s receipt check-in is awaiting coordinator action" : "Confirm Luis received the approved handoff") : roseShift?.handoff_status === "awaiting_caregiver_approval" ? (jordanHandoffApproval?.status === "response_received" ? "Jordan approved the handoff; verify her response" : jordanHandoffApproval ? "Jordan’s handoff review is awaiting coordinator action" : "Request Jordan’s approval of the prepared handoff") : "Prepare the continuity handoff"),
      ...
      unknown: stage === "start" && !visitStarted ? (jordanReadiness?.status === "response_received" ? ["Coordinator verification is still required before Jordan proceeds"] : ["Whether Jordan reviewed the current brief and is ready to begin"]) : stage === "complete" && !visitCompleted ? (jordanVisitOutcome?.status === "response_received" ? ["Coordinator verification is still required before the visit record becomes continuity evidence"] : ["What Jordan observed and completed during the visit"]) : stage === "handoff" && !handoffAcknowledged ? ["Whether the next caregiver received and acknowledged continuity context"] : [],
      human_owner: stage === "handoff" ? "Jordan and Luis" : "Care coordinator and Jordan"
    });
  }
  const blockers = step === 0 ? (evelynResolved ? [] : [evelynInquiry?.status === "response_received" ? "Review Luis’s response and close Evelyn’s verification exception." : evelynInquiry ? "Approve the prepared check-in and review Luis’s response." : "Investigate Evelyn’s missing verification record by checking in with Luis."])
    ...
    : step === 3 ? (roseShift?.visit_status === "in_progress" || roseShift?.visit_status === "completed" ? [] : [jordanReadiness?.status === "response_received" ? "Verify Jordan’s readiness response and clear the visit to begin." : jordanReadiness ? "Send the prepared readiness check-in and review Jordan’s response." : "Prepare a current-brief readiness check-in for Jordan."])
    : step === 4 ? (roseShift?.visit_status === "completed" ? [] : [jordanVisitOutcome?.status === "response_received" ? "Verify Jordan’s submitted visit record before preparing the handoff." : jordanVisitOutcome ? "Send the visit-update request and review Jordan’s response." : "Prepare a bounded visit-update request for Jordan."])
    : (roseShift?.handoff_status === "acknowledged" ? [] : [roseShift?.handoff_status === "available_to_next_caregiver" ? (luisHandoffReceipt?.status === "response_received" ? "Verify Luis’s handoff-receipt response." : luisHandoffReceipt ? "Send the receipt check-in to Luis and review his response." : "Prepare a handoff-receipt check-in for Luis.") : roseShift?.handoff_status === "awaiting_caregiver_approval" ? (jordanHandoffApproval?.status === "response_received" ? "Verify Jordan’s handoff-approval response." : jordanHandoffApproval ? "Send the handoff review to Jordan and review her response." : "Prepare a handoff-approval check-in for Jordan.") : "Prepare the continuity handoff from verified visit evidence."]);
  return {
    step,
...
async function prepareTeamInquiry(db: D1Database, runId: string, body: Record<string, unknown>) {
  ...
  const state = await loadCareState(db, runId);
  if (!state?.care_team_day) return json({ error: "Care Team Day is not active." }, 409);
  ...
  const isEvelynVerification = state.care_team_day.step === 0 && resident === "evelyn-demo" && parsed.data.caregiver_id === "caregiver-luis";
  const isJordanReadiness = state.care_team_day.step === 3 && resident === "rose-demo" && parsed.data.caregiver_id === "caregiver-jordan" && roseShift?.assigned_caregiver_id === "caregiver-jordan" && roseShift.visit_status === "not_started";
  const isJordanVisitOutcome = state.care_team_day.step === 4 && resident === "rose-demo" && parsed.data.caregiver_id === "caregiver-jordan" && roseShift?.assigned_caregiver_id === "caregiver-jordan" && roseShift.visit_status === "in_progress";
  const activeHandoff = state.shift_handoffs.find((handoff) => handoff.shift_id === "shift-wed-pm" && handoff.status !== "dismissed");
  const isJordanHandoffApproval = state.care_team_day.step === 5 && resident === "rose-demo" && parsed.data.caregiver_id === "caregiver-jordan" && activeHandoff?.status === "awaiting_caregiver_approval";
  const isLuisHandoffReceipt = state.care_team_day.step === 5 && resident === "rose-demo" && parsed.data.caregiver_id === "caregiver-luis" && activeHandoff?.status === "available_to_next_caregiver";
  if (!isEvelynVerification && !isJordanReadiness && !isJordanVisitOutcome && !isJordanHandoffApproval && !isLuisHandoffReceipt) return json({ error: "This inquiry must address the assigned caregiver and the active care-team decision." }, 409);
  const inquiryType = isLuisHandoffReceipt ? "handoff_receipt" : isJordanHandoffApproval ? "handoff_approval" : isJordanVisitOutcome ? "visit_outcome" : isJordanReadiness ? "shift_readiness" : "visit_verification";
  ...
  const existing = await db.prepare("SELECT inquiry_id FROM care_demo_team_inquiries WHERE run_id = ? AND inquiry_type = ? AND status != 'dismissed' LIMIT 1").bind(runId, inquiryType).first<{ inquiry_id: string }>();
  if (existing) return json({ error: inquiryType === "handoff_receipt" ? "A Luis handoff-receipt check-in is already active." : inquiryType === "handoff_approval" ? "A Jordan handoff-approval check-in is already active." : inquiryType === "visit_outcome" ? "A Jordan visit-update request is already active." : inquiryType === "shift_readiness" ? "A Jordan readiness check-in is already active." : "A visit-verification inquiry is already active." }, 409);
  const inquiryId = `inq_${crypto.randomUUID()}`;
  const evidence = inquiryType === "handoff_receipt"
    ? { event_type: "handoff_receipt_inquiry_prepared", summary: "Luis handoff-receipt check-in prepared", detail: "The agent staged a request for Luis to confirm receipt of the approved handoff. It did not acknowledge on his behalf.", source: "WebMCP tool · coordinator approval required", trust_boundary: "agent_prepared_handoff_receipt_inquiry", plan_version: "v4" }
    : inquiryType === "handoff_approval"
    ? { event_type: "handoff_approval_inquiry_prepared", summary: "Jordan handoff-review check-in prepared", detail: "The agent staged a request for Jordan to review the prepared handoff. It did not approve on her behalf.", source: "WebMCP tool · coordinator approval required", trust_boundary: "agent_prepared_handoff_approval_inquiry", plan_version: "v4" }
    : inquiryType === "visit_outcome"
    ? { event_type: "visit_outcome_inquiry_prepared", summary: "Jordan visit-update request prepared", detail: "The agent staged a bounded request for Jordan's completed tasks and observations. Nothing was recorded as visit evidence until the coordinator reviews Jordan's response.", source: "WebMCP tool · coordinator approval required", trust_boundary: "agent_prepared_visit_outcome_inquiry", plan_version: "v4" }
    : inquiryType === "shift_readiness"
    ? { event_type: "shift_readiness_inquiry_prepared", summary: "Jordan readiness check-in prepared", detail: "The agent staged a current-brief readiness question for Jordan. Nothing was sent and no visit was started until the coordinator reviews the check-in.", source: "WebMCP tool · coordinator approval required", trust_boundary: "agent_prepared_shift_readiness_inquiry", plan_version: "v4" }
    : { event_type: "visit_verification_inquiry_prepared", summary: "Luis check-in prepared", detail: "The agent staged a bounded operational question. Nothing was sent until the coordinator reviews it.", source: "WebMCP tool · coordinator approval required", trust_boundary: "agent_prepared_team_inquiry", plan_version: "v3" };
  await db.batch([
    db.prepare("INSERT INTO care_demo_team_inquiries (run_id, inquiry_id, resident_id, caregiver_id, inquiry_type, prompt, status, response_code, response_detail, idempotency_key, created_at, responded_at, resolved_at) VALUES (?, ?, ?, ?, ?, ?, 'awaiting_coordinator_approval', NULL, NULL, ?, ?, NULL, NULL)").bind(runId, inquiryId, resident, parsed.data.caregiver_id, inquiryType, parsed.data.prompt, parsed.data.idempotency_key, state.resident.simulated_time),
    evidenceInsert(db, runId, { resident_id: resident!, event_type: evidence.event_type, severity: "attention", summary: evidence.summary, detail: evidence.detail, source: evidence.source, occurred_at: state.resident.simulated_time, actor_type: "agent", actor_id: "page-agent", evidence_type: "prepared_action", observed_at: state.resident.simulated_time, recorded_at: state.resident.simulated_time, trust_boundary: evidence.trust_boundary, plan_version: evidence.plan_version }),
...
async function resolveTeamInquiry(db: D1Database, runId: string, inquiryId: string, body: Record<string, unknown>) {
  ...
  const isReadiness = inquiry.inquiry_type === "shift_readiness";
  const isVisitOutcome = inquiry.inquiry_type === "visit_outcome";
  const isHandoffApproval = inquiry.inquiry_type === "handoff_approval";
  const isHandoffReceipt = inquiry.inquiry_type === "handoff_receipt";
  const responseCode = isHandoffReceipt ? "handoff_received" : isHandoffApproval ? "handoff_approved" : isVisitOutcome ? "visit_record_submitted" : isReadiness ? "ready_for_visit" : "arrived_verification_failed";
  const responseDetail = isHandoffReceipt
    ? "Luis reports: ‘I received and reviewed Rose’s handoff, including the completed routine, later-dinner observation, and unresolved nurse-review item. I have the context I need for the next visit. Can you verify my receipt?’"
    : isHandoffApproval ? "Jordan reports: ‘I reviewed the prepared handoff. It accurately separates completed tasks, my direct observation, and the unresolved nurse-review item. I approve it for Luis. Can you verify my approval?’"
    : isVisitOutcome
    ? "Jordan reports: ‘I completed Rose’s expected evening routine and confirmed the meal delivery. Rose asked to have dinner later than usual. I recorded that preference without assigning a cause or clinical meaning. The unresolved nurse-review item remains open. Can you verify that you received my visit record?’"
    : isReadiness ? "Jordan reports: ‘I reviewed Rose’s current brief, Care Plan v4, the changes since my last visit, and the unresolved nurse-review item. I am at Rose’s residence and ready to begin. Can you verify that you see my acknowledgement?’"
    : "Luis reports: ‘I arrived at 9:08 AM. My EVV application would not complete check-in, so I began the scheduled visit and am with Evelyn now.’";
  await db.batch([
    db.prepare("UPDATE care_demo_team_inquiries SET status = 'response_received', response_code = ?, response_detail = ?, responded_at = ? WHERE run_id = ? AND inquiry_id = ? AND status = 'awaiting_coordinator_approval'").bind(responseCode, responseDetail, respondedAt, runId, inquiryId),
    evidenceInsert(db, runId, { resident_id: inquiry.resident_id, event_type: isHandoffReceipt ? "handoff_receipt_response" : isHandoffApproval ? "handoff_approval_response" : isVisitOutcome ? "caregiver_visit_outcome_response" : isReadiness ? "caregiver_shift_readiness_response" : "caregiver_visit_status_response", severity: "routine", summary: isHandoffReceipt ? "Luis confirmed handoff receipt" : isHandoffApproval ? "Jordan approved the handoff" : isVisitOutcome ? "Jordan submitted a bounded visit record" : isReadiness ? "Jordan responded to the readiness check-in" : "Luis responded to the verification inquiry", detail: responseDetail, source: inquiry.caregiver_id === "caregiver-jordan" ? "Jordan · simulated caregiver self-report" : "Luis · simulated caregiver self-report", occurred_at: respondedAt, actor_type: "caregiver", actor_id: inquiry.caregiver_id, evidence_type: "self_report", observed_at: respondedAt, recorded_at: respondedAt, trust_boundary: isHandoffReceipt ? "caregiver_handoff_receipt_pending_coordinator_verification" : isHandoffApproval ? "caregiver_handoff_approval_pending_coordinator_verification" : isVisitOutcome ? "caregiver_visit_record_pending_coordinator_verification" : isReadiness ? "caregiver_readiness_self_report_pending_coordinator_verification" : "caregiver_self_report_not_independent_evv", plan_version: inquiry.inquiry_type === "visit_verification" ? "v3" : "v4" }),
...
async function closeTeamInquiry(db: D1Database, runId: string, inquiryId: string) {
  ...
  if (inquiry.status !== "response_received") return json({ error: "Review the caregiver response before recording a disposition." }, 409);
  if (inquiry.inquiry_type === "handoff_approval" || inquiry.inquiry_type === "handoff_receipt") {
    const handoff = await db.prepare("SELECT * FROM care_demo_shift_handoffs WHERE run_id = ? AND shift_id = 'shift-wed-pm' AND status != 'dismissed' ORDER BY created_at DESC LIMIT 1").bind(runId).first<DbShiftHandoff>();
    if (!handoff) return json({ error: "Active handoff not found." }, 404);
    const isApproval = inquiry.inquiry_type === "handoff_approval";
    if (isApproval && (inquiry.caregiver_id !== handoff.from_caregiver_id || handoff.status !== "awaiting_caregiver_approval")) return json({ error: "Jordan's handoff is no longer awaiting approval verification." }, 409);
    if (!isApproval && (inquiry.caregiver_id !== handoff.to_caregiver_id || handoff.status !== "available_to_next_caregiver")) return json({ error: "Luis's handoff is no longer awaiting receipt verification." }, 409);
    const verifiedAt = plusMinutes(inquiry.responded_at ?? run.simulated_time, 2);
    const statements = [
      db.prepare("UPDATE care_demo_team_inquiries SET status = 'resolved', resolved_at = ? WHERE run_id = ? AND inquiry_id = ? AND status = 'response_received'").bind(verifiedAt, runId, inquiryId),
      isApproval
        ? db.prepare("UPDATE care_demo_shift_handoffs SET status = 'available_to_next_caregiver', resolved_at = ? WHERE run_id = ? AND shift_handoff_id = ? AND status = 'awaiting_caregiver_approval'").bind(verifiedAt, runId, handoff.shift_handoff_id)
        : db.prepare("UPDATE care_demo_shift_handoffs SET status = 'acknowledged' WHERE run_id = ? AND shift_handoff_id = ? AND status = 'available_to_next_caregiver'").bind(runId, handoff.shift_handoff_id),
      isApproval
        ? db.prepare("UPDATE care_demo_shifts SET handoff_status = 'available_to_next_caregiver', version = version + 1 WHERE run_id = ? AND shift_id = 'shift-wed-pm' AND handoff_status = 'awaiting_caregiver_approval'").bind(runId)
        : db.prepare("UPDATE care_demo_shifts SET handoff_status = 'acknowledged', version = version + 1 WHERE run_id = ? AND shift_id = 'shift-wed-pm' AND handoff_status = 'available_to_next_caregiver'").bind(runId),
      evidenceInsert(db, runId, { resident_id: "rose-demo", event_type: isApproval ? "handoff_approval_verified" : "handoff_receipt_verified", severity: "routine", summary: isApproval ? "Jordan’s handoff approval verified" : "Luis’s handoff receipt verified", detail: isApproval ? "The coordinator verified Jordan’s approval response before making the handoff available to Luis." : "The coordinator verified Luis’s receipt response and closed the continuity loop.", source: "Care coordinator · explicit human verification", occurred_at: verifiedAt, actor_type: "caregiver", actor_id: "coordinator-demo", evidence_type: "human_decision", observed_at: verifiedAt, recorded_at: verifiedAt, trust_boundary: isApproval ? "coordinator_verified_outgoing_caregiver_approval" : "coordinator_verified_next_caregiver_receipt", plan_version: "v4" }),
      db.prepare("UPDATE care_demo_runs SET simulated_time = ?, evidence_version = evidence_version + 1, updated_at = ? WHERE run_id = ?").bind(verifiedAt, verifiedAt, runId)
    ];
    if (!isApproval) statements.splice(3, 0, db.prepare("INSERT OR IGNORE INTO care_demo_handoff_acknowledgements (run_id, acknowledgement_id, shift_handoff_id, caregiver_id, acknowledged_at) VALUES (?, ?, ?, ?, ?)").bind(runId, `ack_${crypto.randomUUID()}`, handoff.shift_handoff_id, inquiry.caregiver_id, verifiedAt));
    await db.batch(statements);
    return json({ state: await loadCareState(db, runId), handoff_approval_verified: isApproval, handoff_receipt_verified: !isApproval, external_side_effect: false });
  }
  if (inquiry.inquiry_type === "visit_outcome") {
    const shift = await db.prepare("SELECT * FROM care_demo_shifts WHERE run_id = ? AND shift_id = 'shift-wed-pm'").bind(runId).first<DbShift>();
    if (!shift || shift.assigned_caregiver_id !== inquiry.caregiver_id || shift.visit_status !== "in_progress") return json({ error: "Jordan's visit is no longer eligible for outcome verification." }, 409);
    const completedAt = plusMinutes(inquiry.responded_at ?? run.simulated_time, 2);
    await db.batch([
      db.prepare("UPDATE care_demo_team_inquiries SET status = 'resolved', resolved_at = ? WHERE run_id = ? AND inquiry_id = ? AND status = 'response_received'").bind(completedAt, runId, inquiryId),
      db.prepare("INSERT INTO care_demo_visit_events (run_id, visit_event_id, shift_id, caregiver_id, event_type, summary, detail, occurred_at, evidence_class) VALUES (?, ?, 'shift-wed-pm', ?, 'routine_completed', 'Evening routine completed', 'Jordan reported completion of the expected non-clinical evening routine; the coordinator verified receipt of the record.', '2026-09-02T19:35:00-04:00', 'caregiver_attestation_plus_coordinator_verification')").bind(runId, `visit_${crypto.randomUUID()}`, inquiry.caregiver_id),
      db.prepare("INSERT INTO care_demo_visit_events (run_id, visit_event_id, shift_id, caregiver_id, event_type, summary, detail, occurred_at, evidence_class) VALUES (?, ?, 'shift-wed-pm', ?, 'meal_delivered', 'Meal delivery acknowledged', 'Jordan reported acknowledging the scheduled meal delivery during the visit.', '2026-09-02T18:10:00-04:00', 'caregiver_attestation')").bind(runId, `visit_${crypto.randomUUID()}`, inquiry.caregiver_id),
      db.prepare("INSERT INTO care_demo_visit_events (run_id, visit_event_id, shift_id, caregiver_id, event_type, summary, detail, occurred_at, evidence_class) VALUES (?, ?, 'shift-wed-pm', ?, 'caregiver_observation', 'Rose requested a later dinner', 'Jordan recorded Rose’s request to move dinner later than her usual routine. No cause or clinical meaning was inferred.', '2026-09-02T18:22:00-04:00', 'caregiver_observation_not_clinical_conclusion')").bind(runId, `visit_${crypto.randomUUID()}`, inquiry.caregiver_id),
      db.prepare("INSERT INTO care_demo_visit_events (run_id, visit_event_id, shift_id, caregiver_id, event_type, summary, detail, occurred_at, evidence_class) VALUES (?, ?, 'shift-wed-pm', ?, 'shift_checked_out', 'Visit check-out recorded', 'The coordinator verified Jordan’s submitted visit record. A structured handoff is now ready to prepare.', ?, 'caregiver_attestation_plus_coordinator_verification')").bind(runId, `visit_${crypto.randomUUID()}`, inquiry.caregiver_id, completedAt),
      db.prepare("UPDATE care_demo_shifts SET visit_status = 'completed', handoff_status = 'ready', version = version + 1 WHERE run_id = ? AND shift_id = 'shift-wed-pm' AND visit_status = 'in_progress'").bind(runId),
      evidenceInsert(db, runId, { resident_id: "rose-demo", event_type: "visit_record_verified", severity: "routine", summary: "Jordan’s visit record verified", detail: "The coordinator reviewed Jordan’s submitted completed tasks, bounded observation, and unresolved item before making the record available for handoff preparation.", source: "Care coordinator · explicit human verification", occurred_at: completedAt, actor_type: "caregiver", actor_id: "coordinator-demo", evidence_type: "human_decision", observed_at: completedAt, recorded_at: completedAt, trust_boundary: "coordinator_verified_caregiver_visit_record", plan_version: "v4" }),
    ...
    return json({ state: await loadCareState(db, runId), visit_record_verified: true, visit_completed: true, external_side_effect: false });
  }
  if (inquiry.inquiry_type === "shift_readiness") {
    const shift = await db.prepare("SELECT * FROM care_demo_shifts WHERE run_id = ? AND shift_id = 'shift-wed-pm'").bind(runId).first<DbShift>();
    if (!shift || shift.assigned_caregiver_id !== inquiry.caregiver_id || shift.coverage_status !== "assigned" || shift.visit_status !== "not_started") return json({ error: "Jordan's assignment is no longer eligible for readiness clearance." }, 409);
    const startedAt = plusMinutes(inquiry.responded_at ?? run.simulated_time, 2);
    await db.batch([
      db.prepare("UPDATE care_demo_team_inquiries SET status = 'resolved', resolved_at = ? WHERE run_id = ? AND inquiry_id = ? AND status = 'response_received'").bind(startedAt, runId, inquiryId),
      db.prepare("UPDATE care_demo_shifts SET visit_status = 'in_progress', version = version + 1 WHERE run_id = ? AND shift_id = 'shift-wed-pm' AND visit_status = 'not_started'").bind(runId),
      db.prepare("INSERT INTO care_demo_visit_events (run_id, visit_event_id, shift_id, caregiver_id, event_type, summary, detail, occurred_at, evidence_class) VALUES (?, ?, 'shift-wed-pm', ?, 'shift_checked_in', 'Visit check-in recorded', 'Jordan reported reviewing the current assignment brief. The coordinator verified the response and cleared the simulated visit to begin.', ?, 'caregiver_attestation_plus_coordinator_verification')").bind(runId, `visit_${crypto.randomUUID()}`, inquiry.caregiver_id, startedAt),
      evidenceInsert(db, runId, { resident_id: "rose-demo", event_type: "shift_readiness_verified", severity: "routine", summary: "Jordan cleared to begin Rose’s visit", detail: "The coordinator verified Jordan’s current-brief acknowledgement and readiness response before the simulated visit began.", source: "Care coordinator · explicit human verification", occurred_at: startedAt, actor_type: "caregiver", actor_id: "coordinator-demo", evidence_type: "human_decision", observed_at: startedAt, recorded_at: startedAt, trust_boundary: "coordinator_verified_caregiver_readiness", plan_version: "v4" }),
    ...
    return json({ state: await loadCareState(db, runId), readiness_verified: true, visit_started: true, external_side_effect: false });
  }
  await db.batch([
